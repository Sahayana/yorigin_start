...
async def test_shop_point_intersects() -> None:
    ...
    # When
    result = await ShopCollection.point_intersects(
...
async def test_shop_point_intersects_when_it_has_multiple_delivery_area() -> (
    None
):
    # Given
    await asyncio.gather(
...
async def test_shop_get_distinct_category_codes_by_point_intersects() -> None:
    ...
    # When
    result_codes = (
        await ShopCollection.get_distinct_category_codes_by_point_intersects(
            GeoJsonPoint(coordinates=[5, 5])
        )
    )
    ...
    assert all(
        ...
        for code in [CategoryCode.CHICKEN, CategoryCode.BURGER]
    )
